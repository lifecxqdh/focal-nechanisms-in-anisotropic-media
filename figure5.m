clear
close all
vel=[2600,1380,2300];% velocity of P and S wave, density
...
for ij=1:kk
    ...
    Rsh(ij)=sqrt(sumsh);
    Rsv(ij)=sqrt(sumsv);
    ratio(ij)=(sums/sump)*3;
end
subplot(1,2,1)
plot(slope,Rp,'LineWidth',1.5)
...
ylabel('Radiation Pattern Coefficient','FontSize',14,'Fontname','Times New Roman')
subplot(1,2,2)
plot(slope,Rs,'LineWidth',1.5)
xticks([-90,-60,-30,0,30,60,90])
set(gca,'XTickLabel',{'-90^{\circ}','-60^{\circ}','-30^{\circ}','0^{\circ}','30^{\circ}','60^{\circ}','90^{\circ}'})
xlim([-90 90])
grid on
